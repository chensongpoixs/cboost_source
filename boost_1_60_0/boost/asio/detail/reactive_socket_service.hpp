...
public:
  ...
  {
    socket_ops::bind(impl.socket_, endpoi+20
		nt.data(), endpoint.size(), ec);
    return ec;
  }
      ...
      socket_base::message_flags flags, Handler& handler)
  {
    ...
    typedef reactive_socket_recvfrom_op<MutableBufferSequence,
        endpoint_type, Handler> op;
    typename op::ptr p = { boost::asio::detail::addressof(handler),
      boost_asio_handler_alloc_helpers::allocate(  // 调用C++的 ::operator new 一个内存 
        sizeof(op), handler), 0 };
  ...
  // Accept a new connection.  // add event epoll 
      ...
      Socket& peer, endpoint_type* peer_endpoint, boost::system::error_code& ec)
  {
    ...
    if (new_socket.get() != invalid_socket)
    {
      ...
      if (!peer.assign(impl.protocol_, new_socket.get(), ec)) // add event epoll_ctl
        new_socket.release();
    }
      ...
      endpoint_type* peer_endpoint, Handler& handler)
  {
    ...
    p.p = new (p.v) op(impl.socket_, impl.state_, peer,
        impl.protocol_, peer_endpoint, handler);  // 调用构造函数
      ...
      const endpoint_type& peer_endpoint, Handler& handler)
  {
    ...
    start_connect_op(impl, p.p, is_continuation,
        peer_endpoint.data(), peer_endpoint.size()); // ::accept 
    p.v = p.p = 0;
  }
};
